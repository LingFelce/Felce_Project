...
GenewiseCounts <- read.table("genes.csv", sep=",", header = TRUE, row.names=NULL)


#Remove other columns (not interested in soluble or adherent)
genes <- GenewiseCounts[,(1:5)]

#Set up groups (metadata)
rest <- genes [, c("R1", "R2")]
stim <- genes [, c("P1", "P2")]

group=rep(c("rest", "stim"),each=2)
...
# Differential gene expression: Resting vs Stimulated Jurkats

``` {r}

fit <- glmQLFit(list, design, robust=TRUE)

restvstim<- makeContrasts(stim-rest, levels=design)
res <- glmQLFTest(fit, contrast=restvstim)
...
tr <- glmTreat(fit, contrast=restvstim, lfc=log2(1.5))
```
...
## Heatmap to show differential gene expression of Resting vs Stimulated Jurkats
...
```{r}
all_res <- topTags(res, n=Inf)
sig_res <- all_res[all_res$table$FDR < 0.05,]
write.csv(all_res, file="edgeR_results/rest_v_stim_res.csv")
write.csv(sig_res, file="edgeR_results/rest_v_stim_sig.csv")

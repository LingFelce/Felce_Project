...
```
## Resting vs 2 hour Activated CD4 T Cells - using DESeq2 gene list
...
library(readr)
sig <- read_csv("results/rest_vs_2h_sig.csv")
...
write.csv(GOgenes, file="GO/rest_vs_2h_BP.csv")


```
### Showing the ego object in graphical form

```{r 2h barplot and dotplot,fig.height=6, fig.width=8}
...
```{r 2h GO terms relationship, fig.height=12, fig.width=14}
emapplot(ego)
...
```{r 2h genes in GO terms, fig.height=20, fig.width=14}
...
plotEnrichment(pathways[["Interleukin-10 signaling"]], ranks)
plotEnrichment(pathways[["Interleukin-4 and Interleukin-13 signaling"]], ranks)
plotEnrichment(pathways[["TNFs bind their physiological receptors"]], ranks)
plotEnrichment(pathways[["Signaling by Interleukins"]], ranks)
plotEnrichment(pathways[["TNFR2 non-canonical NF-kB pathway"]], ranks)
plotEnrichment(pathways[["Interleukin-1 family signaling"]], ranks)
plotEnrichment(pathways[["Cytokine Signaling in Immune system"]], ranks)
plotEnrichment(pathways[["Metabolism of RNA"]], ranks)
plotEnrichment(pathways[["Immune System"]], ranks)
...
plotEnrichment(pathways[["HALLMARK_TGF_BETA_SIGNALING"]],ranks)
plotEnrichment(pathways[["HALLMARK_EPITHELIAL_MESENCHYMAL_TRANSITION"]],ranks)
plotEnrichment(pathways[["HALLMARK_KRAS_SIGNALING_DN"]],ranks)
plotEnrichment(pathways[["HALLMARK_COAGULATION"]],ranks)
...
plotEnrichment(pathways[["HALLMARK_IL2_STAT5_SIGNALING"]],ranks)
plotEnrichment(pathways[["HALLMARK_UV_RESPONSE_UP"]],ranks)
plotEnrichment(pathways[["HALLMARK_APOPTOSIS"]],ranks)
plotEnrichment(pathways[["HALLMARK_CHOLESTEROL_HOMEOSTASIS"]],ranks)
plotEnrichment(pathways[["HALLMARK_MYOGENESIS"]],ranks)
plotEnrichment(pathways[["HALLMARK_P53_PATHWAY"]],ranks)
plotEnrichment(pathways[["HALLMARK_KRAS_SIGNALING_UP"]],ranks)
plotEnrichment(pathways[["HALLMARK_ALLOGRAFT_REJECTION"]],ranks)
plotEnrichment(pathways[["HALLMARK_HYPOXIA"]],ranks)
plotEnrichment(pathways[["HALLMARK_ESTROGEN_RESPONSE_EARLY"]],ranks)
plotEnrichment(pathways[["HALLMARK_ESTROGEN_RESPONSE_LATE"]],ranks)
...
## Resting vs 24 hour Activated CD4 T Cells - using DESeq2 gene list
...
library(readr)
sig <- read_csv("results/rest_vs_24h_sig.csv")
...
write.csv(GOgenes, file="GO/rest_vs_24h_BP.csv")


```
### Showing the ego object in graphical form

```{r 24h barplot and dotplot,fig.height=6, fig.width=8}
barplot (ego, showCategory = 10)
...
```{r 24h GO terms relationship, fig.height=12, fig.width=14}
emapplot(ego)
...
```{r 24h genes in GO terms, fig.height=20, fig.width=14}
#logFC is optional, but if available will colour-code the nodes
logFC <- sig$log2FoldChange
#set name of object
names(logFC) <- sig$Entrez.Gene
...
#visualise GSEA results for some of these pathways - top 10

plotEnrichment(pathways[["Gap junction assembly"]], ranks)
plotEnrichment(pathways[["Unwinding of DNA"]], ranks)
plotEnrichment(pathways[["Activation of ATR in response to replication stress"]], ranks)
plotEnrichment(pathways[["Extension of Telomeres"]], ranks)
plotEnrichment(pathways[["Telomere C-strand (Lagging Strand) Synthesis"]], ranks)
plotEnrichment(pathways[["Lagging Strand Synthesis"]], ranks)
plotEnrichment(pathways[["Leading Strand Synthesis"]], ranks)
plotEnrichment(pathways[["Polymerase switching"]], ranks)
plotEnrichment(pathways[["Polymerase switching on the C-strand of the telomere"]], ranks)
plotEnrichment(pathways[["Activation of the pre-replicative complex"]], ranks)
...
plotEnrichment(pathways[["HALLMARK_UNFOLDED_PROTEIN_RESPONSE"]],ranks)
plotEnrichment(pathways[["HALLMARK_MYC_TARGETS_V1"]],ranks)
plotEnrichment(pathways[["HALLMARK_UV_RESPONSE_UP"]],ranks)
plotEnrichment(pathways[["HALLMARK_IL2_STAT5_SIGNALING"]],ranks)
plotEnrichment(pathways[["HALLMARK_MTORC1_SIGNALING"]],ranks)
plotEnrichment(pathways[["HALLMARK_TNFA_SIGNALING_VIA_NFKB"]],ranks)
plotEnrichment(pathways[["HALLMARK_DNA_REPAIR"]],ranks)
